...
class ImageAdmin(PrimitivePermissionAwareModelAdmin):
    list_display = ('label',)
    list_per_page = 10
    search_fields = ['name', 'original_filename','default_alt_text','default_caption','notes', 'author']
    ...
    def render_change_form(self, request, context, add=False, change=False, form_url='', obj=None):
        ...
        return super(ImageAdmin, self).render_change_form(
            request=request, context=context, add=False, change=False, form_url=form_url, obj=obj)
    ...
    def delete_view(self, request, object_id, extra_context=None):
        ...
        response = super(ImageAdmin, self).delete_view(request=request,
    ...
    def add_view(self, request):
        return HttpResponseRedirect(
            reverse('admin:image_filer-directory_listing-root'))
...
class FolderAdmin(PrimitivePermissionAwareModelAdmin):
    ...
    def response_change(self, request, obj):
        ...
        instead of the default change_list_view
        """
        response = super(FolderAdmin, self).response_change(request, obj)
        if response['Location']:
            # it was a successful save
    ...
    def delete_view(self, request, object_id, extra_context=None):
        ...
        response = super(FolderAdmin, self).delete_view(request=request,
            object_id=object_id, extra_context=extra_context)
        url = response.get("Location", None)
        if url in ["../../../../","../../"]:
            if parent_folder:
...
class ClipboardItemInline(admin.TabularInline):
    ...
    verbose_name_plural = _("Clipboards items")
